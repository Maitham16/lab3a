// T5: Render example3.xml with shadows.
...
#include "classes/material_solid.h"
#include <cmath>
#include <pugixml.hpp>
#include <string>
#include <iostream>
...
void parseScene(const std::string &fileName, std::string &outputFileName, Color &bgColor, Camera &camera, std::vector<anotherSphere> &spheres, Light &light, Color &ambientLight)
{
    ...
    double fov = 90.0;
    double scale = tan(0.4 * fov * 3.14 / 180.0);
    Vec3 cameraPosition(0, 0, 3);

    std::vector<Light> lights;
    lights.push_back(Light(Vec3(100, 4, 10), Color(1, 1, 1)));
...
int main()
{
    ...
    // parse the scene
    parseScene("scenes/example3.xml", outputFileName, bgColor, camera, spheres, light, ambientLight);
