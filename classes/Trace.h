...
Color trace(const Ray &ray, const std::vector<Sphere> &scene, const std::vector<Light> &lights, int maxDepth)
{
    if (maxDepth <= 0)
    {
    ...
    if (intersectionFound)
    {
        ...
        Material material = scene[sphereIndex].material;
        ...
        Color reflectedColor = trace(reflectedRay, scene, lights, maxDepth - 1);

        // Calculate the final color
        color += reflectedColor * material.specular_color;
